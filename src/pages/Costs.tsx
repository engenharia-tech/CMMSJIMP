    ...
    <ErrorBoundary>
      <div className="space-y-8">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold text-slate-900 tracking-tight">{t('cost_management')}</h2>
            <p className="text-slate-500 mt-1">{t('costs_desc_full', 'Track and analyze your maintenance expenditures.')}</p>
          </div>
          <button 
            onClick={handleExportCosts}
            className="flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-xl font-bold shadow-lg shadow-slate-900/20 hover:bg-slate-800 transition-all active:scale-95"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="font-bold text-slate-900">{t('total_cost', 'Total Cost')}</h3>
            </div>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="font-bold text-slate-900">{t('labor_cost', 'Mão de Obra')}</h3>
            </div>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="font-bold text-slate-900">{t('parts_cost', 'Peças')}</h3>
            </div>
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="font-bold text-slate-900">{t('avg_per_order', 'Avg per Order')}</h3>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
            <h3 className="text-lg font-bold text-slate-900 mb-6">{t('cost_by_part', 'Custo por Peça (Top 10)')}</h3>
          ...
          <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
            <h3 className="text-lg font-bold text-slate-900 mb-6">{t('cost_by_sector', 'Custo por Setor')}</h3>
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm lg:col-span-2">
            <h3 className="text-lg font-bold text-slate-900 mb-6">{t('recent_expenses', 'Despesas Recentes')}</h3>
              ...
              {orders.slice(0, 6).map((order) => (
                <div key={order.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 mt-0.5">
                        <span className="text-[9px] font-black uppercase tracking-widest px-1.5 py-0.5 bg-blue-100 text-blue-600 rounded">
                          L: R$ {order.labor_cost?.toLocaleString('pt-BR')}
                        </span>
                        <span className="text-[9px] font-black uppercase tracking-widest px-1.5 py-0.5 bg-amber-100 text-amber-600 rounded">
                  ...
                  <div className="text-right">
                    <p className="font-black text-slate-900">R$ {order.maintenance_cost.toLocaleString('pt-BR')}</p>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{order.request_date}</p>
                  </div>
                </div>
              ))}
            </div>
