    ...
    <ErrorBoundary>
      <div className="space-y-8">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold text-slate-900 tracking-tight">{t('parts_inventory')}</h2>
            <p className="text-slate-500 mt-1">{t('manage_parts_desc', 'Manage your spare parts and stock levels.')}</p>
          </div>
          <button 
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">{t('part', 'Part')}</th>
                <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">{t('stock', 'Stock')}</th>
                <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">{t('unit_cost', 'Unit Cost')}</th>
                <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-widest">{t('supplier', 'Supplier')}</th>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500 font-medium uppercase tracking-wider">{part.part_code}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <span className={`font-bold ${part.stock_quantity <= part.minimum_stock ? 'text-red-600' : 'text-slate-700'}`}>
                        {part.stock_quantity}
                      </span>
          ...
          {filteredParts.length === 0 && !loading && (
            <div className="p-20 text-center">
              <p className="text-slate-500">{t('no_parts_found', 'No parts found.')}</p>
            </div>
          )}
        </div>
